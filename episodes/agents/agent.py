...
def _build_agent() -> Agent[RecoveryDeps, RecoveryAgentResult]:
    ...
    # Enable OTel instrumentation when tracing is active
    ...
    with tempfile.TemporaryDirectory(prefix="ragtime-recovery-") as download_dir:
        async with recovery_browser(download_dir) as page:
            ...
            result = await asyncio.wait_for(
                _run_with_otel(agent, system_prompt, deps, event),
                timeout=timeout,
            )
...
async def _run_with_otel(agent, system_prompt, deps, event):
    """Run the agent with OTel tracing when enabled.

    Creates a dedicated span for the recovery agent run, detached from
    any parent pipeline step context so it gets its own trace.
    """
    from .. import telemetry
        ...
        try:
            ...
            try:
                ...
                attributes = {
                    ...
                    "ragtime.recovery.error_type": event.error_type or "",
                    "ragtime.recovery.attempt_number": str(event.attempt_number),
                }
...
def _flush_otel():
    """Flush buffered OTel traces so they reach the exporter."""
    from .. import telemetry
